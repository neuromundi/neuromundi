...
export function KProviderRegister({ typeKey, onSuccess, complete = false }: { typeKey: KType; onSuccess?: () => void; complete?: boolean }) {
  ...
  const submit = async () => {
    ...
    const payload = {
      ...
      role: 'provider' as const, providerType: typeKey, isCompany: true, businessName: name.trim(),
  ...
  if (done) {
    return (
      <div className="rounded-xl bg-brand-50 p-4 text-center text-sm text-slate-700">
        ...
        {typeKey !== 'ngo' && <p className="mt-2 text-sm font-medium text-brand-700">{t('reg.afterConfirmPay')}</p>}
      </div>
    );
  }
